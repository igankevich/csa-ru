module Jekyll
  class Scholar
    ...
    module Utilities
      ...
      def bibliography_tag(entry, index, context)
        return missing_reference unless entry

		tmp = context.dup
        tmp['entry'] = liquidify(entry)
        tmp['reference'] = reference_tag(entry, index)
        tmp['key'] = entry.key
        tmp['type'] = entry.type.to_s
        tmp['link'] = repository_link_for(entry)
        tmp['links'] = repository_links_for(entry)
        tmp['index'] = index
        tmp['details'] = details_link_for(entry)

        liquid_template.render(tmp)
      end
